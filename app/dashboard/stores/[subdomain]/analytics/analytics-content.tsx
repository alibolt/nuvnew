...
export function AnalyticsContent({ subdomain }: AnalyticsContentProps) {
  ...
  const fetchAnalytics = async () => {
    ...
    setError(null);
    try {
      const endpoint = `/api/stores/${subdomain}/analytics/${activeTab}?period=${dateRange}`;
  ...
  useEffect(() => {
    fetchAnalytics();
  }, [subdomain, dateRange, activeTab]);

  // Chart colors
  const COLORS = ['#008060', '#D4A5A5', '#8B9F7E', '#F59E0B', '#3B82F6', '#8B5CF6'];

  // Format currency
  const formatCurrency = (value: number) => formatPrice(value, 'USD', 'en-US');
