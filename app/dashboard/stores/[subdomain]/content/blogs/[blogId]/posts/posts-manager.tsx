...
export default function PostsManager({ subdomain, blogId, blogTitle }: PostsManagerProps) {
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch(`/api/stores/${subdomain}/blogs/${blogId}/posts`);
      if (response.ok) {
  ...
  const handleDelete = async (postId: string) => {
    ...
    try {
      const response = await fetch(`/api/stores/${subdomain}/blogs/${blogId}/posts/${postId}`, {
        method: 'DELETE',
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link
            href={`/dashboard/stores/${subdomain}/content/blogs`}
        ...
        <Button
          onClick={() => router.push(`/dashboard/stores/${subdomain}/content/blogs/${blogId}/posts/new`)}
                ...
                <tr key={post.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/dashboard/stores/${subdomain}/content/blogs/${blogId}/posts/${post.id}`}
