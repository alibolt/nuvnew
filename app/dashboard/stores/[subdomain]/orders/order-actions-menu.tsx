...
import { MoreVertical, Edit, Truck, CreditCard, X, RefreshCw, Archive, DollarSign } from 'lucide-react';
...
export function OrderActionsMenu({ order, subdomain }: OrderActionsMenuProps) {
  ...
  const handleStatusUpdate = async (newStatus: string, statusType: 'order' | 'financial' | 'fulfillment') => {
    ...
    try {
      ...
      const response = await fetch(`/api/stores/${subdomain}/orders/${order.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleFulfillOrder = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/stores/${subdomain}/orders/${order.id}/fulfill`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const getAvailableActions = () => {
    ...
    // Refund action - only show if payment is paid or partially_refunded
    if ((order.financialStatus === 'paid' || order.financialStatus === 'partially_refunded') && order.paymentIntentId) {
      actions.push({
        label: 'Issue Refund',
        icon: DollarSign,
        onClick: () => {
