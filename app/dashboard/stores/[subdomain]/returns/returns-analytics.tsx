...
export function ReturnsAnalytics({ subdomain, returns, onBack }: ReturnsAnalyticsProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const topReasons = Object.entries(reasonBreakdown)
      ...
      .map(([reason, count]) => ({
        ...
        count,
        percentage: (((count as number) / (Object.values(reasonBreakdown) as number[]).reduce((sum: number, c: number) => sum + c, 0)) * 100).toFixed(1)
      }));
    ...
    const repeatCustomers = (Object.values(customerReturns) as number[]).filter(count => count > 1).length;
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-20 bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-blue-600 h-2 rounded-full" 
                      style={{ 
                        width: `${Math.max(5, ((count as number) / analytics.totalReturns) * 100)}%` 
                      }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-10">
                    {(((count as number) / analytics.totalReturns) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-3">
            {analytics.topReasons.map((reason, index) => (
              <div key={index} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-900">{reason.count as number}</span>
