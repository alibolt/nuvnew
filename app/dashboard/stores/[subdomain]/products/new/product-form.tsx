...
export function ProductForm({ subdomain, product, isEdit = false, onSuccess }: ProductFormProps) {
  ...
  const handleAutoSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/stores/${subdomain}/products/${product.id}`, {
        method: 'PATCH',
  ...
  useEffect(() => {
    if (isEdit && product) {
      console.log('🔍 ProductForm Edit Mode - Product data:', product);
      ...
      setTrackQuantity(product.trackQuantity ?? true);
      setContinueSellingWhenOutOfStock(product.continueSellingWhenOutOfStock ?? false);
      
      // Handle tags (parse JSON if needed)
      ...
      // Handle product images (parse JSON if needed)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        variantCombinations.map(async (variant) => {
          ...
          for (const image of variant.images) {
            ...
            if (uploadRes.ok) {
              ...
              console.error('Failed to upload variant image:', await uploadRes.text());
            }
          }
      ...
      const url = isEdit 
        ? `/api/stores/${subdomain}/products/${product.id}`
        : `/api/stores/${subdomain}/products`;
      ...
      if (!response.ok) {
        ...
        if (onSuccess) {
          ...
          console.log('🔄 Navigating to:', `/dashboard/stores/${subdomain}/products`);
          console.log('🔍 Subdomain value:', subdomain, typeof subdomain);
          router.push(`/dashboard/stores/${subdomain}/products`);
          router.refresh();
        }
      }
    } catch (err) {
