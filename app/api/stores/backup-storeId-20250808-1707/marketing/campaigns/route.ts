...
export async function GET(
  ...
  try {
    ...
    let campaigns = (storeSettings?.marketingCampaigns as any[]) || [];
...
export async function POST(
  ...
  try {
    ...
    const newCampaign = {
      ...
      type: campaignType,
      ...validation.data,
      status: validation.data.scheduling.type === 'immediate' ? 'sending' : 'scheduled',
      audienceSize,
      stats: {
        sent: 0,
        delivered: 0,
    ...
    if (validation.data.scheduling.type === 'immediate') {
