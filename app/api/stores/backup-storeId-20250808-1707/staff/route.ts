...
export async function GET(
  ...
  try {
    ...
    // Get staff members from store settings
    ...
    return NextResponse.json({ staffMembers: allStaff });
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    const existingMember = staffMembers.find(
      member => member.email === validation.data.email
    );

    if (existingMember) {
      return NextResponse.json({ 
        error: 'This email is already a staff member' 
      }, { status: 400 });
    }
    ...
    const newStaffMember = {
      ...
      twoFactorAuth: validation.data.twoFactorAuth || { enabled: false, method: 'app' },
    ...
    return NextResponse.json({ 
      ...
      staffMember: {
        ...newStaffMember,
        inviteToken: undefined // Don't expose token
      }
    });
  } catch (error) {
