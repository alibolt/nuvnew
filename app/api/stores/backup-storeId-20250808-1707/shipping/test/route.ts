      ...
      const coverageResults = destinations.map(destination => {
        ...
        return {
          ...
          zones: applicableZones.map(zone => ({
            id: zone.id,
            name: zone.name,
            methodCount: zone.methods.filter(m => m.enabled).length
          })),
          methodCount: applicableZones.reduce(
            (sum, zone) => sum + zone.methods.filter(m => m.enabled).length, 
            0
          )
        };
      });
    ...
    for (const zone of applicableZones) {
      for (const method of zone.methods) {
        ...
        try {
          ...
          errors.push(
            `Error calculating rate for "${method.name}" in zone "${zone.name}": ${error.message}`
          );
        }
      }
    }
