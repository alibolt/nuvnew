...
export async function POST(
  ...
  try {
    ...
    if (validation.data.amount > maxRefundable) {
      return NextResponse.json({ 
        error: `Cannot refund ${validation.data.amount}. Maximum refundable: ${maxRefundable}` 
      }, { status: 400 });
    }
    ...
    if (validation.data.lineItems) {
      for (const refundItem of validation.data.lineItems) {
        ...
        if (!lineItem) {
          return NextResponse.json({ 
            error: `Line item ${refundItem.lineItemId} not found` 
          }, { status: 400 });
        }
        if (refundItem.quantity > lineItem.quantity) {
          return NextResponse.json({ 
            error: `Cannot refund ${refundItem.quantity} of ${lineItem.title}. Only ${lineItem.quantity} purchased.` 
          }, { status: 400 });
        }
      }
    }
    ...
    const updatedOrder = await prisma.$transaction(async (tx) => {
      ...
      if (validation.data.restock && store.storeSettings?.stockSettings?.trackInventory) {
