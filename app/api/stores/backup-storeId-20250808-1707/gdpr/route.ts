...
export async function GET(
  ...
  try {
    ...
    const gdprSettings = storeSettings?.gdprSettings as any || defaultGdprSettings;
...
export async function POST(
  ...
  try {
    ...
    if (!action || (!customerEmail && !customerId)) {
      return NextResponse.json({ 
        error: 'Action and customer email or ID are required' 
      }, { status: 400 });
    }
    ...
    const dataRequests = storeSettings?.dataRequests as any[] || [];
