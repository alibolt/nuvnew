...
async function getReviewAnalytics(subdomain: string) {
  const [
    totalReviews,
    averageRating,
    ratingDistribution,
    ...
    prisma.productReview.count({
      where: { product: { store: { subdomain } } }
    }),

    // Average rating across all products
    prisma.productReview.aggregate({
      where: { product: { store: { subdomain } }, status: 'approved' },
      _avg: { rating: true }
    }),
    ...
    // Pending reviews
    prisma.productReview.count({
      where: { product: { store: { subdomain } }, status: 'pending' }
    }),
