...
async function getSystemStatus(storeId: string): Promise<any> {
  ...
  const services = [
    ...
    },
    {
      ...
      status: (store.storeSettings as any)?.paymentMethods ? 'operational' : 'degraded',
    ...
    {
      name: 'Search Engine',
      status: (store.storeSettings as any)?.searchSettings ? 'operational' : 'warning',
  ...
  try {
    ...
    if (widget) {
      ...
      switch (widget) {
        ...
        default:
          return apiResponse.badRequest('Unknown widget type');
      }
    }
    ...
    const dashboardData = {
      ...
      system: systemStatus,
      timeRange,
      refreshInterval: (storeSettings as any)?.monitoringConfig?.refreshInterval || 300
    };
