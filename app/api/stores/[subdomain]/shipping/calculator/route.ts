...
export async function PUT(
  ...
  try {
    ...
    const zonesWithIds = validation.data.map((zone: any, zoneIndex: number) => ({
      ...zone,
      id: `zone_${Date.now()}_${zoneIndex}_${Math.random().toString(36).substr(2, 9)}`,
      methods: zone.methods.map((method: any, methodIndex: number) => ({
        ...method,
        id: method.id || `method_${Date.now()}_${methodIndex}_${Math.random().toString(36).substr(2, 9)}`
      }))
    }));
...
async function getShippingAnalytics(subdomain: string) {
  // Get store first
