...
export async function GET(
  ...
  try {
    ...
    if (action === 'countries') {
      return apiResponse.success({ countries: COMMON_COUNTRIES });
    }
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        activeMethods: shippingZones.reduce((sum, z) => 
          sum + z.methods.filter((m: any) => m.enabled).length, 0
        )
      }
    });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    if (!zoneId) {
      return apiResponse.badRequest('Zone ID is required');
    }
    ...
    if (currentZones.length === updatedZones.length) {
      return apiResponse.notFound('Zone ');
    }
...
async function getZoneAnalytics(subdomain: string, zoneId: string) {
