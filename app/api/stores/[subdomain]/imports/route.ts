...
export async function POST(
  ...
  try {
    ...
    if (!validation.data.fileUrl && !validation.data.fileData) {
      return apiResponse.badRequest('Either fileUrl or fileData must be provided');
    }
...
export async function PUT(
  ...
  try {
    ...
    if (!importId || !action) {
      return apiResponse.badRequest('Import ID and action are required');
    }
    ...
    if (jobIndex === -1) {
      return apiResponse.notFound('Import job ');
    }
    ...
    switch (action) {
      case 'cancel':
        if (importJobs[jobIndex].status === 'pending' || 
          ...
          return apiResponse.badRequest('Cannot cancel completed or failed import');
        }
        break;
      ...
      case 'retry':
        if (importJobs[jobIndex].status === 'failed') {
          ...
          return apiResponse.badRequest('Can only retry failed imports');
        }
        break;
      ...
      case 'update_mappings':
        if (mappings && (importJobs[jobIndex].status === 'pending' || 
          ...
          return apiResponse.badRequest('Can only update mappings for pending imports');
        }
        break;
...
export async function DELETE(
  ...
  try {
    ...
    if (!importId) {
      return apiResponse.badRequest('Import ID is required');
    }
    ...
    return apiResponse.success({ message: 'Import job deleted successfully' });
  } catch (error) {
