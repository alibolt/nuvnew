...
// GET - Get language settings (public endpoint for reading)
export async function GET(
  ...
  try {
    ...
    const languageSettings = (storeSettings as any)?.languageSettings || {
      enabledLanguages: ['en'],
      defaultLanguage: 'en',
      autoDetect: false,
    ...
    if (type === 'translations') {
      ...
      const translations = (storeSettings as any)?.translations || {};
...
export async function PUT(
  ...
  try {
    ...
    if (invalidLanguages.length > 0) {
      return apiResponse.badRequest('Unsupported languages: ${invalidLanguages.join(', ')}');
    }
    ...
    if (!validation.data.enabledLanguages.includes(validation.data.defaultLanguage)) {
      return apiResponse.badRequest('Default language must be in enabled languages');
    }
