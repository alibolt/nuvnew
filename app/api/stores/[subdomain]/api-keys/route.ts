...
export async function POST(
  ...
  try {
    ...
    if (existingKey) {
      return apiResponse.badRequest('An API key with this name already exists');
    }
...
export async function PUT(
  ...
  try {
    ...
    if (keyIndex === -1) {
      return apiResponse.notFound('API key ');
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!keyId) {
      return apiResponse.badRequest('API key ID is required');
    }
    ...
    return apiResponse.success({ message: 'API key deleted successfully' });
  } catch (error) {
