...
export async function POST(
  ...
  try {
    ...
    if (order.status === 'cancelled') {
      return apiResponse.badRequest('Cannot fulfill cancelled order');
    }

    if (order.fulfillmentStatus === 'fulfilled') {
      return apiResponse.badRequest('Order is already fulfilled');
    }
    ...
    if (validation.data.lineItems) {
      ...
      for (const requestedItem of validation.data.lineItems) {
        ...
        if (!lineItem) {
          return apiResponse.badRequest('Line item ${requestedItem.lineItemId} not found');
        }
        if (requestedItem.quantity > lineItem.quantity) {
          return apiResponse.badRequest('Cannot fulfill ${requestedItem.quantity} of ${lineItem.title}. Only ${lineItem.quantity} ordered.');
        }
      }
    }
