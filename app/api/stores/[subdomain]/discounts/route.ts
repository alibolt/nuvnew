...
export async function GET(
  ...
  try {
    ...
    if (storeSettings?.discounts) {
      try {
        // Handle both JSON string and object formats
        discounts = typeof storeSettings.discounts === 'string' 
          ? JSON.parse(storeSettings.discounts)
          : storeSettings.discounts;
          
        // Ensure it's an array
        if (!Array.isArray(discounts)) {
          console.error('[DISCOUNTS API] Discounts is not an array:', typeof discounts);
          discounts = [];
        }
      } catch (error) {
        console.error('[DISCOUNTS API] Error parsing discounts:', error);
        discounts = [];
      }
    }
...
export async function POST(
  ...
  try {
    ...
    if (existingDiscount) {
      return apiResponse.badRequest('A discount with this code already exists');
    }
...
export async function PUT(
  ...
  try {
    ...
    if (!discountId) {
      return apiResponse.badRequest('Discount ID is required');
    }
