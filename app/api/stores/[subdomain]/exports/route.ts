...
export async function POST(
  ...
  try {
    ...
    const newExportJob = {
      ...
      columns: validation.data.columns || (exportTypes as any)[validation.data.type]?.defaultColumns || [],
      ...
      try {
        ...
        if (jobIndex !== -1) {
          ...
          currentJobs[jobIndex] = {
            ...
            downloadUrl: `/api/stores/${subdomain}/exports/${newExportJob.id}/download`,
...
export async function PUT(
  ...
  try {
    ...
    if (!exportId || !action) {
      return apiResponse.badRequest('Export ID and action are required');
    }
    ...
    if (jobIndex === -1) {
      return apiResponse.notFound('Export job ');
    }
    ...
    switch (action) {
      case 'cancel':
        if (exportJobs[jobIndex].status === 'pending' || exportJobs[jobIndex].status === 'processing') {
          ...
          return apiResponse.badRequest('Cannot cancel completed or failed export');
        }
        break;
      ...
      case 'retry':
        if (exportJobs[jobIndex].status === 'failed') {
          ...
          return apiResponse.badRequest('Can only retry failed exports');
        }
        break;
...
export async function DELETE(
  ...
  try {
    ...
    if (!exportId) {
      return apiResponse.badRequest('Export ID is required');
    }
    ...
    return apiResponse.success({ message: 'Export job deleted successfully' });
  } catch (error) {
