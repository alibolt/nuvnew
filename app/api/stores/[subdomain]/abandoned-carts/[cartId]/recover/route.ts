...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ subdomain: string; cartId: string }> }
) {
  try {
    ...
    const { subdomain, cartId } = await params;
    ...
    if (cartIndex === -1) {
      return apiResponse.notFound('Cart ');
    }
    ...
    if (!cart.email) {
      return apiResponse.badRequest('Cannot send recovery email - no email address available');
    }

    // Check if cart is already recovered
    if (cart.status === 'recovered') {
      return apiResponse.badRequest('Cart has already been recovered');
    }
...
export async function GET(
  ...
  try {
    ...
    return apiResponse.success(recoveryStatus);
  } catch (error) {
