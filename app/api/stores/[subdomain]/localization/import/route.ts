...
function parsePO(content: string): any[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('msgctxt')) {
      if (current) current.context = trimmed.match(/"(.*)"/)?.[1] || null;
      ...
      if (current) {
        ...
        if (trimmed.includes('[')) {
          ...
          if (index) current.pluralForms[index] = value;
        } else {
          current.value = value;
        }
      }
...
export async function POST(
  ...
  try {
    ...
    const options = (validation.data.options || {}) as any;
    ...
    await prisma.storeSettings.update({
      where: { storeId: store.id },
      data: { translations: newTranslations } as any
    });
...
msgstr "Añadir al carrito"`,
    ...
    return NextResponse.json({
      template: (examples as any)[format] || examples.json,
