...
export async function POST(
  ...
  try {
    ...
    switch (action) {
      ...
      case 'move':
        if (!data?.folder) {
          return apiResponse.badRequest('Folder name is required for move operation');
        }
      ...
      case 'tag':
        for (const mediaId of mediaIds) {
          ...
          if (fileIndex !== -1) {
            ...
            // Remove tags
            if (data?.removeTags) {
              currentTags = currentTags.filter((tag: any) => !data.removeTags!.includes(tag));
            }
      ...
      case 'update':
        if (!data?.metadata) {
          return apiResponse.badRequest('Metadata is required for update operation');
        }
...
export async function PUT(
  ...
  try {
    ...
    switch (action) {
      ...
      case 'rename':
        if (!newName) {
          return apiResponse.badRequest('New name is required for rename operation');
        }
