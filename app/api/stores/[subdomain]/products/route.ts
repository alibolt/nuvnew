...
export async function GET(
  ...
  try {
    ...
    console.log('[PRODUCTS API] Request for subdomain:', subdomain);
    console.log('[PRODUCTS API] Session user:', session?.user?.email, session?.user?.id);
    ...
    if (!isPublic) {
      if (!session?.user?.id) {
        console.log('[PRODUCTS API] No session found');
        return apiResponse.unauthorized();
      }

      // Check if user owns the store or has staff access
      const isOwner = store.userId === session.user.id;
      
      // For now, only check ownership. Staff member system needs proper user relation
      if (!isOwner) {
        // Check if user has staff access via email
        const hasStaffAccess = await prisma.staffMember.findFirst({
          where: {
            storeId: store.id,
        ...
        if (!hasStaffAccess) {
          console.log('[PRODUCTS API] User', session.user.email, 'cannot access store', subdomain);
          console.log('[PRODUCTS API] Store owner:', store.userId, 'Current user:', session.user.id);
          return apiResponse.unauthorized('Not authorized to access this store');
        }
      }
    }
    ...
    console.log('[PRODUCTS API] Search params:', { search, active, isPublic });
...
export async function POST(
  ...
  try {
    ...
    const { subdomain } = await params;
    const store = await prisma.store.findFirst({
      where: { subdomain: subdomain, userId: session.user.id },
    });
    ...
    await triggerSearchIndexUpdate(store.id, 'product', product.id, 'create');
    ...
    if (error.code === 'P2002') {
      ...
      return apiResponse.badRequest('Unique constraint failed: This ${target} is already in use.');
    }
