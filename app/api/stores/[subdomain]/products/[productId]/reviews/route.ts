...
// GET /api/stores/[subdomain]/products/[productId]/reviews
  ...
  try {
    ...
    const analytics = {
      ...
      verifiedCount: await prisma.productReview.count({
        where: { productId, verified: true, product: { storeId: store.id } }
      }),
      withImagesCount: await prisma.productReview.count({
        where: { productId, images: { not: Prisma.JsonNull }, product: { storeId: store.id } }
      }),
      statusCounts: await prisma.productReview.groupBy({
        by: ['status'],
        where: { productId, product: { storeId: store.id } },
        _count: { status: true }
...
// POST /api/stores/[subdomain]/products/[productId]/reviews
export async function POST(
  ...
  try {
    ...
    if (!reviewSettings.allowGuestReviews && !reviewData.customerId) {
      return apiResponse.badRequest('Only registered customers can leave reviews');
    }
    ...
    if (reviewData.customerId) {
      ...
      if (existingReview) {
        return apiResponse.badRequest('You have already reviewed this product');
      }
    }
    ...
    if (reviewSettings.requirePurchase && reviewData.customerId) {
      ...
      if (!purchaseExists) {
        return apiResponse.badRequest('You must purchase this product before reviewing it');
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (!reviewId) {
      return apiResponse.badRequest('Review ID is required');
    }
