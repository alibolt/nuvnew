...
// Helper function to sync policy with page
async function syncPolicyWithPage(storeId: string, policy: any) {
  try {
    // Map policy types to page slugs
    ...
    const slug = slugMap[policy.type];
    if (!slug) return;
    ...
    if (policy.enabled) {
      ...
      await prisma.page.upsert({
        ...
        update: {
          ...
          isPublished: policy.published !== false,
          seoTitle: policy.metaTitle || policy.title,
          seoDescription: policy.metaDescription || `${policy.title} for our store`,
          updatedAt: new Date()
        },
        create: {
          storeId: storeId,
      ...
      // Delete the page if policy is disabled
    ...
    console.error('Error syncing policy with page:', error);
    // Don't throw - let the policy update succeed even if page sync fails
  }
}
...
export async function GET(
  ...
  try {
    ...
    let policies = (storeSettings as any)?.returnPolicy as any[] || [];
...
// Helper function to sync all policies with pages
async function syncAllPoliciesWithPages(storeId: string, policiesData: any) {
  const policyTypes = [
    { type: 'refund_policy', slug: 'refund-policy', data: policiesData.refundPolicy },
    { type: 'privacy_policy', slug: 'privacy-policy', data: policiesData.privacyPolicy },
    { type: 'terms_of_service', slug: 'terms-of-service', data: policiesData.termsOfService },
    { type: 'shipping_policy', slug: 'shipping-policy', data: policiesData.shippingPolicy }
  ];
    ...
    try {
      if (policy.data.enabled) {
        ...
        await prisma.page.upsert({
          ...
          update: {
            title: policy.data.title || policy.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        ...
        // Delete page if policy is disabled
        await prisma.page.deleteMany({
          where: {
            storeId: storeId,
            slug: policy.slug
      ...
      console.error(`Error syncing ${policy.type} with page:`, error);
    }
  }
}
...
export async function POST(
  ...
  try {
    ...
    const policies = (storeSettings as any)?.returnPolicy as any[] || [];

    // Check for duplicate slug
    const existingPolicy = policies.find(policy => policy.slug === validation.data.slug);
    if (existingPolicy) {
      return apiResponse.badRequest('A policy with this slug already exists');
    }
...
export async function PUT(
  ...
  try {
    ...
    // Check if this is from the policies form (has policies object)
    if (body.policies) {
      // This is from the settings form
      const store = await prisma.store.findFirst({
        where: {
          subdomain: subdomain,
      ...
      // Update store settings with policies data
      await prisma.storeSettings.upsert({
        where: { storeId: store.id },
        update: {
          // Store the policies data in appropriate fields
          returnPolicy: body.policies // Store all policies data
        },
        create: {
          storeId: store.id,
          returnPolicy: body.policies
        }
      });

      // Sync all policies with pages
      await syncAllPoliciesWithPages(store.id, body.policies);
    ...
    if (policyIndex === -1) {
      return apiResponse.notFound('Policy ');
    }
    ...
    // Sync with pages if enabled status changed
    if (updateData.enabled !== undefined) {
      await syncPolicyWithPage(store.id, policies[policyIndex]);
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!policyId) {
      return apiResponse.badRequest('Policy ID is required');
    }
    ...
    await prisma.storeSettings.update({
      where: { storeId: store.id },
      data: {
        returnPolicy: filteredPolicies
      }
    });

    return apiResponse.success({ message: 'Policy deleted successfully' });
  } catch (error) {
